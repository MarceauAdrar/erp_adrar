...
$sql = "SELECT stagiaire_id, stagiaire_nom, stagiaire_prenom, id_session   
        FROM stagiaires 
...
$req = $db->prepare($sql);
$req->bindParam(":id_session", $stagiaire['id_session']);
$req->execute();
$stagiaires = $req->fetchAll(PDO::FETCH_COLUMN);
...
if (!$stagiaire) {
    ...
    if (isset($_GET['mode']) && !empty($_GET['mode']) && $_GET['mode'] === "edit" && $_SESSION["utilisateur"]["formateur_id"] > 0) {
        $key = array_search($stagiaire['stagiaire_id'], $stagiaires);

        if ($key !== false && isset($stagiaires[$key + 1])) {
            $next_trainee = $stagiaires[array_keys($stagiaires)[$key + 1]];
        } else {
            $next_trainee = null;
        }

        if ($key !== false && isset($stagiaires[$key - 1])) {
            $previous_trainee = $stagiaires[array_keys($stagiaires)[$key - 1]];
        } else {
            $previous_trainee = null;
        }

        $title = " | Évaluation par stagiaire";
        include_once('./header.php');
        echo '
            <div class="fixed-top bg-white z-1 row text-center" style="margin-top: 8.8%;">
                <div class="col-2 mt-3">
                    <a href="' . (isset($previous_trainee) && !empty($previous_trainee) ? 'viewer.php?mode=edit&id_stagiaire=' . $previous_trainee : "#") . '" class="btn btn-dark border border-light' . (isset($previous_trainee) && !empty($previous_trainee) ? "" : " disabled") . '" role="button"><</a>
                </div>
                <div class="col-8 text-center">
                    <h1>Évaluation du stagiaire: ' . $stagiaire['stagiaire_prenom'] . "&nbsp;" . $stagiaire['stagiaire_nom'] . '</h1>
                    <div>
                ...
                <div class="col-2 mt-3">
                    <a href="' . (isset($next_trainee) && !empty($next_trainee) ? 'viewer.php?mode=edit&id_stagiaire=' . $next_trainee : "#") . '" class="btn btn-dark border border-light' . (isset($next_trainee) && !empty($next_trainee) ? "" : " disabled") . '" role="button">></a>
                </div> 
            </div>
            ' . $tbl . '
            ';
