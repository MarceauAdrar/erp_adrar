...
if ($req_lignes->rowCount() > 0) {
    foreach ($req_lignes->fetchAll(PDO::FETCH_ASSOC) as $eval) {
        ...
        if(empty($eval["id_stagiaire"])) {
            ...
            $lignes .= '    <td><a class="btn btn-info" id="eval_' . $eval["evaluation_id"] . '" href="http://' . $_SERVER["SERVER_NAME"] . '/erp/public/formation/resolve.php?token=' . $eval["evaluation_token"] . '">Reprendre l\'évaluation</a></td>';
