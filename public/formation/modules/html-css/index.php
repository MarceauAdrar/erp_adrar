...
$lignes = "";
$sql_lignes = "SELECT evaluation_id, evaluation_title, evaluation_goals, evaluation_token, evaluation_errors_max, stagiaire_evaluation_completed, stagiaire_evaluation_correction, stagiaire_evaluation_errors_found, id_stagiaire
                FROM evaluations e
...
if ($req_lignes->rowCount() > 0) {
    foreach ($req_lignes->fetchAll(PDO::FETCH_ASSOC) as $eval) {
        ...
        if(empty($eval["id_stagiaire"])) {
            ...
            $lignes .= '    <td><a class="btn btn-info" id="eval_' . $eval["evaluation_id"] . '" href="http://' . $_SERVER["SERVER_NAME"] . '/evaluations/public/resolve.php?token=' . $eval["evaluation_token"] . '">Reprendre l\'évaluation</a></td>';
            ...
            $lignes .= '    <td><a class="btn btn-dark" href="http://' . $_SERVER["SERVER_NAME"] . '/evaluations/public/achieved.php?module=html-css&tp=' . $eval["evaluation_id"] . '">Voir mes erreurs</a></td>';
        } else {
...
<div class="container">
    <div class="row">
        <div class="col-8 offset-2 text-center">
            ...
            <img class="svgs-full" src="http://<?=$_SERVER["SERVER_NAME"]?>/public/imgs/inspection.svg" alt="Illustration code review" />
