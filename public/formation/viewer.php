...
$req->bindParam(":document_index", $_GET['document']);
$req->execute();
...
$req->bindParam(":id_stagiaire", $_GET['id_stagiaire']);
$req->execute();
...
$req->bindParam(":id_formateur", $_SESSION['utilisateur']['formateur_id']);
$req->execute();
        ...
        if (!empty($page['document_page_textes_ajoutes'])) {
            foreach (json_decode($page['document_page_textes_ajoutes'], true) as $key => $i) {
                ...
                if (isset($i['signature']) && !empty($i['signature'])) { // Gère la signature du formateur si elle est nécessaire sur le document
                    ...
                    $html2pdf->Image(__DIR__ . "/../../src/v/formateurs/" . $formateur[$i['signature']], $i['xsignature'], $i['ysignature'], 47, 27.5);
                }
            }
        }
